...
export async function POST(request: Request) {
	try {
		...
		if (!notificationId) {
			console.error(
				"Failed to get notification ID from created notification"
			);
			return NextResponse.json(
				{
		...
		const { data: anggotaData, error: anggotaError } =
			await supabaseAdmin
				.from("anggota")
				.select("id")
				.eq("is_active", true);
		...
		// Create read status entries for all anggota using UPSERT to handle duplicates
		if (anggotaData && anggotaData.length > 0) {
			...
			const { error: readError } = await supabaseAdmin
				...
				.upsert(readEntries, {
					onConflict: "global_notifikasi_id,anggota_id",
				});

			if (readError) {
				console.error(
					"Error creating notification read entries:",
					readError
				);
				return NextResponse.json(
					{
