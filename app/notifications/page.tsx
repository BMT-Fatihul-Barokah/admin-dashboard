...
export default function NotificationsPage() {
	...
	const [announcementNotifications, setAnnouncementNotifications] =
		useState<CombinedNotification[]>([]);
	const [jatuhTempoNotifications, setJatuhTempoNotifications] = useState<
		CombinedNotification[]
	>([]);
	const [loading, setLoading] = useState(true);
	const [activeTab, setActiveTab] = useState("all");
	const [selectedNotification, setSelectedNotification] =
		useState<CombinedNotification | null>(null);
	const [detailOpen, setDetailOpen] = useState(false);

	// Create notification dialog states
	const [createDialogOpen, setCreateDialogOpen] = useState(false);
	const [anggotaList, setAnggotaList] = useState<
		{ id: string; nama: string; nomor_rekening: string }[]
	>([]);
		...
		try {
			if (formData.source === "global") {
				...
				toast({
					title: "Info",
					description:
						"Notifikasi transaksi biasanya dibuat secara otomatis oleh sistem.",
					variant: "default",
				});
				return;
			}
			...
			toast({
				title: "Error",
				description:
					"Gagal membuat notifikasi. Silakan coba lagi nanti.",
				variant: "destructive",
			});
		}
	};
					...
					<DialogContent className="sm:max-w-[500px]">
						<DialogHeader>
							<div className="flex items-center gap-2 mb-1">
								<div
									className={`flex h-10 w-10 items-center justify-center rounded-full ${getNotificationIconBg(
										selectedNotification.jenis ||
											"info"
									)}`}
								>
									{getNotificationIcon(
							...
							<div className="flex items-center gap-1">
								<Badge className="bg-slate-100 text-slate-800">
									{selectedNotification.jenis
										.charAt(0)
										.toUpperCase() +
										selectedNotification.jenis.slice(
											1
										)}
								</Badge>
							</div>
			...
			>
				<DialogContent className="sm:max-w-[600px]">
					<DialogHeader>
						...
						<DialogDescription>
							Buat notifikasi baru untuk ditampilkan
							di dashboard atau dikirim ke anggota
							tertentu.
						</DialogDescription>
					</DialogHeader>
					<div className="grid gap-4 py-4">
						...
						<div className="grid grid-cols-4 items-center gap-4">
							...
							<div className="col-span-3">
								<Popover
									open={openAnggotaPopover}
									onOpenChange={
										setOpenAnggotaPopover
									}
								>
														...
														.filter(
															(
																...
																anggota.nama
																	.toLowerCase()
																	.includes(
																		anggotaSearchQuery.toLowerCase()
																	) ||
																anggota.nomor_rekening
																	.toLowerCase()
																	.includes(
																		anggotaSearchQuery.toLowerCase()
																	)
							...
							{loading ? (
								...
								notifications.map(
									(notification) => (
										...
										>
											<div
												className={`rounded-full p-2 ${getNotificationIconBg(
													notification.jenis ||
														"info"
												)}`}
											>
				...
				<TabsContent value="jatuh_tempo" className="space-y-4">
					<Card>
						...
						<CardContent className="space-y-4">
							...
							) : jatuhTempoNotifications.length ===
							  0 ? (
								<div className="flex flex-col items-center justify-center py-8">
									...
									<p className="text-sm text-muted-foreground">
										Notifikasi jatuh
										tempo akan muncul di
										sini
									</p>
								</div>
							) : (
								jatuhTempoNotifications.map(
									(notification) => (
										<div
											key={
												notification.id
