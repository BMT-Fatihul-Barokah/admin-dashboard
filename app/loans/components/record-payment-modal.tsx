...
import {
	...
	recordPembayaranPembiayaan,
	generatePaymentSchedule,
	getPembayaranByPembiayaanId,
	PembayaranPembiayaan,
} from "@/lib/pembayaran-pinjaman";
import {
	Card,
	CardContent,
	CardDescription,
...
export function RecordPaymentModal({
	...
	const handleMonthSelection = (month: number) => {
		...
		const monthItem = paymentSchedule.find(
			(item) => item.bulan_ke === month
		);
		if (monthItem) {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		if (paymentAmount > Number(loan.sisa_pembayaran)) {
			setError(
				`Jumlah pembayaran tidak boleh melebihi sisa pembayaran (${formatCurrency(
					Number(loan.sisa_pembayaran)
				)})`
			);
			return;
		}
		...
		try {
			...
			const result = await recordPembayaranPembiayaan(
				{
					...
					catatan:
						notes ||
						`Pembayaran angsuran bulan ke-${selectedMonth} via ${paymentMethod}${
							referenceNumber
								? ` (${referenceNumber})`
								: ""
						}`,
			...
			if (result.success) {
				...
				const sisaPembayaran =
					result.data?.sisa_pembayaran || 0;
				const sisaBulan = result.data?.sisa_bulan || 0;
				const status = result.data?.status || "aktif";

				toast.success(
					`Pembayaran berhasil dicatat. Sisa pembayaran: ${formatCurrency(
						sisaPembayaran
					)}. ` +
				...
				<form onSubmit={handleSubmit}>
					<div className="grid gap-4 py-4">
						...
						<div className="grid gap-2">
							<Label htmlFor="installment-month">
								Angsuran Bulan Ke-
							</Label>
							<Select
