	...
	return new Promise((resolve, reject) => {
		...
		reader.onload = (e) => {
			try {
				...
				// Convert to JSON
				const jsonData =
					XLSX.utils.sheet_to_json<AnggotaExcelRow>(
						worksheet
					);
				resolve(jsonData);
			} catch (error) {
...
export async function parseTransactionExcelFile(
	file: File
): Promise<TransaksiExcelRow[]> {
	return new Promise((resolve, reject) => {
		const reader = new FileReader();
		...
		reader.onload = (e) => {
			try {
				...
				// Convert to JSON
				const jsonData =
					XLSX.utils.sheet_to_json<TransaksiExcelRow>(
						worksheet
					);
				resolve(jsonData);
			} catch (error) {
...
export async function importAnggotaData(
	...
	try {
		...
		try {
			...
			console.log(
				"RPC function check failed, assuming it does not exist"
			);
			rpcExists = false;
		}
		...
		for (let i = 0; i < data.length; i++) {
			...
			try {
				...
				const { data: existingAnggota, error: fetchError } =
					await supabase
						.from("anggota")
				...
				const anggotaData = {
					...
					tanggal_lahir: formatDate(
						excelDateToJSDate(row["Tanggal Lahir"])
					),
				...
				if (existingAnggota) {
					...
					console.log(
						`Updating account ${row["No. Rekening"]} with new balance: ${row["Saldo"]}`
					);
					console.log(
						"Current balance in database:",
						existingAnggota.saldo
					);
					...
					if (rpcExists) {
						console.log(
							"Using RPC function to update balance"
						);
						const rpcResponse = await supabase.rpc(
							"update_anggota_balance",
							{
								p_nomor_rekening:
									row["No. Rekening"],
					...
					if (!updateSuccess) {
						...
						console.log(
							"Trying standard update by nomor_rekening"
						);
					...
					if (verifyError) {
						...
						if (
							...
							console.log(
								"❌ Balance not updated correctly. Expected:",
								Number(row["Saldo"]),
								"Actual:",
								verifyData.saldo
							);
							console.log(
								"The update appeared to succeed but the balance was not updated. This is likely due to RLS policies."
							);
						}
					}
					...
					if (updateSuccess) {
						...
						throw new Error(
							`Failed to update account ${row["No. Rekening"]} after multiple attempts`
						);
					}
				} else {
					// Create new anggota
		...
		result.message = `Failed to import data: ${
...
export async function importTransactionData(
	...
	try {
		...
		console.log(
			`Processing ${data.length} transactions for date: ${
				startOfDay.toISOString().split("T")[0]
			}`
		);

		// Check for existing transactions today to avoid duplicates
		const { data: existingTransactions, error: fetchError } =
			await supabase
				.from("transaksi")
				.select(
					"id, anggota_id, tipe_transaksi, source_type, jumlah, created_at"
				)
		...
		if (fetchError) {
			console.error(
				"Error fetching existing transactions:",
				fetchError
			);
		...
		console.log(
			...
			} existing transactions for today`
		);
		...
		for (let i = 0; i < data.length; i++) {
			...
			try {
				...
				if (typeof row["Jumlah"] === "number") {
					...
					if (isNaN(amount)) {
						throw new Error(
							`Invalid amount format: ${row["Jumlah"]}`
						);
					}
				}
				...
				if (row["Tanggal"]) {
					console.log(
						`Processing date from Excel: ${row["Tanggal"]}`
					);

					if (typeof row["Tanggal"] === "number") {
						// Excel date format (serial number)
						transactionDate = excelDateToJSDate(
							row["Tanggal"]
						);
						console.log(
							`Parsed Excel serial date: ${transactionDate.toISOString()}`
						);
						...
						if (dateParts.length === 3) {
							...
							console.log(
								`Parsing date parts: day=${day}, month=${monthKey}(${month}), year=${year}`
							);
							...
							if (
								...
								console.log(
									`Parsed Indonesian date format: ${transactionDate.toISOString()}`
								);
							} else {
								console.error(
									`Invalid date parts: day=${day}, month=${month}, year=${year}`
								);
								throw new Error(
									`Invalid date format: ${row["Tanggal"]}`
								);
							}
						} else {
							...
							if (
								...
								console.error(
									`Failed to parse date: ${row["Tanggal"]}`
								);
								throw new Error(
									`Invalid date format: ${row["Tanggal"]}`
								);
							...
							console.log(
								`Parsed standard date format: ${transactionDate.toISOString()}`
							);
						}
					} else {
						// Default to today if date type is unexpected
						console.log(
							`Unexpected date type: ${typeof row[
								"Tanggal"
							]}, using today's date`
						);
						transactionDate = new Date();
					...
					console.log(
						"No date provided in Excel, using today's date"
					);
					transactionDate = new Date();
					transactionDate.setHours(12, 0, 0, 0);
				}
				...
				console.log(
					`Looking for member: "${row["Nama Anggota"]}"`
				);

				// Try exact match first
				let { data: anggotaData, error: anggotaError } =
					await supabase
						.from("anggota")
				...
				if (anggotaError || !anggotaData) {
					console.log(
						`No exact match found for "${row["Nama Anggota"]}", trying partial match...`
					);
					const {
					...
					if (partialMatchError || !partialMatchData) {
						console.error(
							`No member found for "${row["Nama Anggota"]}", error:`,
							partialMatchError
						);
						throw new Error(
							`Anggota not found: ${row["Nama Anggota"]}`
						);
					}

					anggotaData = partialMatchData;
					console.log(
						`Found member via partial match: ${anggotaData.nama} (ID: ${anggotaData.id})`
					);
				} else {
					console.log(
						`Found member via exact match: ${anggotaData.nama} (ID: ${anggotaData.id})`
					);
				}
				...
				if (
					loanTypes.some((type) =>
						accountOrLoanName.includes(type)
					)
				) {
					isLoanTransaction = true;
					console.log(
						`This appears to be a loan transaction for: ${accountOrLoanName}`
					);

					// Find the loan by type and member - first try active loans
					let { data: loanData, error: loanError } =
						await supabase
							.from("pinjaman")
							.select(
								"id, jenis_pinjaman, sisa_pembayaran, total_pembayaran, status"
							)
					...
					const tipeTransaksi =
						row["Jenis Transaksi"] &&
						typeof row["Jenis Transaksi"] === "string"
							? row[
									"Jenis Transaksi"
							  ].toLowerCase() === "keluar"
								? "keluar"
					...
					source_type =
						sourceTypeMap[source_type] || "lainnya";
					...
					if (
						...
						console.log(
							`No active loan found for member: ${anggotaData.nama}, loan type: ${accountOrLoanName}. Trying to find any loan...`
						);
						...
						if (
							anyLoanError ||
							!anyLoanData ||
							anyLoanData.length === 0
						) {
							console.error(
								`No loan found for member: ${anggotaData.nama}, loan type: ${accountOrLoanName}, error:`,
								anyLoanError
							);
							...
							if (
								...
								throw new Error(
									`No loan found for member: ${row["Nama Anggota"]} with type: ${accountOrLoanName}`
								);
							}
						}
						...
						console.log(
							`Found non-active loan: ${loanData[0].id} with status ${loanData[0].status}`
						);
					}
					...
					console.log(
						`Using loan: ${pinjamanData.id} with status ${pinjamanData.status} and remaining balance ${pinjamanData.sisa_pembayaran}`
					);

					// If the loan is already paid off (status = 'lunas') and this is a payment, warn about it
					if (
						pinjamanData.status === "lunas" &&
						source_type === "pembayaran_pinjaman"
					) {
						console.warn(
							`Warning: Payment for an already paid off loan: ${pinjamanData.id} for member ${anggotaData.nama}`
						);
					}
					console.log(
						`Found loan: ${pinjamanData.id} with remaining balance ${pinjamanData.sisa_pembayaran}`
					);
				} else {
					// This is a savings account transaction
					console.log(
						`Looking for savings account for member ID: ${anggotaData.id} with name: ${accountOrLoanName}`
					);

					// Try to find the specific savings account by name
					const {
						data: matchingTabungan,
						error: matchingTabunganError,
					} = await supabase
						.from("tabungan")
						.select(
							"id, saldo, jenis_tabungan_id, jenis_tabungan(nama)"
						)
						.eq("anggota_id", anggotaData.id)
						.eq("status", "aktif")
					...
					if (
						matchingTabunganError ||
						!matchingTabungan ||
						matchingTabungan.length === 0
					) {
						console.error(
							`No savings accounts found for member: ${anggotaData.nama}, error:`,
							matchingTabunganError
						);
						throw new Error(
							`No savings accounts found for member: ${row["Nama Anggota"]}`
						);
					}
					...
					for (const account of matchingTabungan) {
						...
						const accountName =
							account.jenis_tabungan?.nama;
						if (
							accountName &&
							accountOrLoanName.includes(
								accountName
							)
						) {
							...
							foundAccount = true;
							console.log(
								`Found matching savings account: ${tabunganData.id} with name ${accountName} and balance ${tabunganData.saldo}`
							);
							break;
						}
					}
					...
					if (!foundAccount) {
						...
						const {
							data: defaultTabungan,
							error: defaultTabunganError,
						} = await supabase
							.from("tabungan")
						...
						if (
							...
							if (matchingTabungan.length > 0) {
								tabunganData =
									matchingTabungan[0];
								console.log(
									`Using first available savings account: ${tabunganData.id} with balance ${tabunganData.saldo}`
								);
							} else {
								throw new Error(
									`No suitable savings account found for member: ${row["Nama Anggota"]}`
								);
							}
						} else {
							tabunganData = defaultTabungan;
							console.log(
								`Using default savings account: ${tabunganData.id} with balance ${tabunganData.saldo}`
							);
						}
					}
				}
				...
				const tipeTransaksi =
					row["Jenis Transaksi"] &&
					typeof row["Jenis Transaksi"] === "string"
						? row["Jenis Transaksi"].toLowerCase() ===
						  "keluar"
							? "keluar"
							: "masuk"
						: "masuk"; // Default to 'masuk' if undefined
				...
				const isDuplicate = existingTransactions?.some((t) => {
					...
					return (
						t.anggota_id === anggotaData.id &&
						t.tipe_transaksi === tipeTransaksi &&
						t.source_type ===
							(isLoanTransaction
								? "pembiayaan"
								: "tabungan") &&
						Math.abs(Number(t.jumlah) - amount) < 0.01
					); // Allow for small floating point differences
				});

				if (isDuplicate) {
					console.log(
						`Potential duplicate transaction detected for ${row["Nama Anggota"]}, ${tipeTransaksi}, ${source_type}, ${amount}`
					);
				...
				if (isLoanTransaction && pinjamanData) {
					...
					if (
						...
						if (newRemainingBalance < 0)
							newRemainingBalance = 0; // Prevent negative balance
					} else if (
						tipeTransaksi === "keluar" &&
						source_type === "pencairan_pinjaman"
					) {
						...
						deskripsi:
							...
							`${
								tipeTransaksi
									.charAt(0)
									.toUpperCase() +
								tipeTransaksi.slice(1)
							} pembiayaan - ${accountOrLoanName}`,
					...
					console.log(
						"Attempting to insert loan transaction with data:",
						transactionData
					);
					...
					if (transactionError) {
						console.error(
							"Loan transaction insert error:",
							transactionError
						);
						throw new Error(
							`Failed to insert loan transaction: ${
					...
					console.log(
						"Attempting to insert savings transaction with data:",
						transactionData
					);
					...
					if (transactionError) {
						console.error(
							"Savings transaction insert error:",
							transactionError
						);
						throw new Error(
							`Failed to insert savings transaction: ${
								transactionError.message ||
								"Unknown error"
							}`
						);
					...
					console.log(
						"Savings transaction inserted successfully:",
						insertedTransaction
					);

					// Update savings account balance
					console.log(
						`Updating savings account ${tabunganData.id} balance from ${tabunganData.saldo} to ${newBalance}`
					);
					...
					if (updateError) {
						console.error(
							"Error updating savings balance:",
							updateError
						);
						throw new Error(
							`Failed to update savings balance: ${
								updateError.message ||
								"Unknown error"
							}`
						);
					...
					console.log(
						"Savings balance updated successfully"
					);
				} else {
					throw new Error(
						`No valid account or loan found for this transaction`
					);
				}
				result.created++;
				...
				if (onProgress) {
					onProgress(
						Math.floor(((i + 1) / data.length) * 100)
					);
				}
			} catch (error: any) {
				console.error(
					`Error processing transaction for ${row["Nama Anggota"]}:`,
					error
				);
		...
		result.success = false;
		result.message = `Failed to import transactions: ${
			error.message || "Unknown error"
		}`;
	}
...
export async function getImportHistory(): Promise<any[]> {
	try {
		...
		if (!tableExists) {
			console.log(
				"Import history table does not exist yet, returning empty array"
			);
			return [];
		}
...
export async function testInsertTransaction(): Promise<any> {
	try {
		...
		if (anggotaError || !anggotaData) {
			...
			return {
				success: false,
				message: "Failed to find a valid anggota",
			};
		}
		...
		if (tabunganError || !tabunganData) {
			...
			return {
				success: false,
				message: "Failed to find a valid tabungan",
			};
		}
		...
		if (transactionError) {
			...
			return {
				success: false,
				message: `Failed to insert transaction: ${transactionError.message}`,
			};
		}
		...
		if (updateError) {
			console.error(
				"Error updating tabungan balance:",
				updateError
			);
			return {
				success: false,
				message: `Transaction inserted but failed to update balance: ${updateError.message}`,
			};
		}
...
export async function recordImportHistory(
	...
	try {
		...
		if (!tableExists) {
			console.log(
				"Skipping import history recording as table does not exist"
			);
			return;
		}
		...
		if (error) {
			const errorMessage =
				...
				"message" in error
					...
					: "Unknown error";
			console.log(
				"Error recording import history (non-critical):",
				errorMessage
			);
		} else {
		...
		console.log(
			"Error in recordImportHistory (non-critical):",
			error?.message || error
		);
	}
}
