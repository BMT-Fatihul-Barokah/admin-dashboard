...
const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL || 'https://vszhxeamcxgqtwyaxhlu.supabase.co';
const supabaseKey = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InZzemh4ZWFtY3hncXR3eWF4aGx1Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDg4NDQ0ODYsImV4cCI6MjA2NDQyMDQ4Nn0.x6Nj5UAHLA2nsNfvK4P8opRkB0U3--ZFt7Dc3Dj-q94';
...
async function testLoanCreation() {
  ...
  try {
    ...
    const { data: jenisPembiayaanData, error: jenisPembiayaanError } = await supabase
    ...
    if (error) {
      console.error('Error calling add_pembiayaan RPC function:', error);
      return;
    }
    ...
    if (data && data.success) {
      console.log('Loan creation successful! Pembiayaan ID:', data.pembiayaan_id);
      ...
      if (pembiayaanError) {
        console.error('Error fetching created pembiayaan:', pembiayaanError);
        return;
      }
