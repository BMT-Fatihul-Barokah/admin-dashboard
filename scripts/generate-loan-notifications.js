...
async function generateDummyLoans() {
	try {
		...
		if (!anggota || anggota.length === 0) {
			console.error(
				"No anggota found. Please create some members first."
			);
			return;
		}
		...
		const { data: jenisPembiayaan, error: jenisPembiayaanError } =
		...
		if (!jenisPembiayaan || jenisPembiayaan.length === 0) {
			console.error(
				"No jenis_pembiayaan found. Please create loan types first."
			);
			return;
		}
		...
		async function createLoanWithNotification(
			anggotaId,
			jenisPembiayaanId,
			tanggalJatuhTempo
		) {
			try {
				...
				const loanAmount =
					Math.floor(Math.random() * 10000000) + 1000000; // Random amount between 1-10 million
				const { data: loanId, error: loanError } =
					await supabase.rpc("admin_create_pembiayaan", {
						p_anggota_id: anggotaId,
						p_jenis_pembiayaan_id: jenisPembiayaanId,
						p_jumlah: loanAmount,
						p_jangka_waktu:
							Math.floor(Math.random() * 24) + 6, // Random between 6-30 months
						p_status: "aktif",
						p_jatuh_tempo: tanggalJatuhTempo
							.toISOString()
				...
				const { data: result, error: transactionError } =
					await supabase.rpc(
						"admin_create_jatuh_tempo_transaction",
						{
							p_anggota_id: anggotaId,
				...
				if (transactionError) {
					console.error(
						"Error creating transaction and notification:",
						transactionError
					);
					return {
						loanId,
					...
					console.log(
						`Created transaction ${result.transaksi_id} and notification ${result.notifikasi_id} for loan ${loanId}`
					);
					return {
						loanId,
				...
				console.error(
					"Error in createLoanWithNotification:",
					error
				);
				return null;
			}
		...
		for (const loan of loans) {
			const result = await createLoanWithNotification(
				loan.anggota_id,
				jenisPembiayaanId,
				new Date(loan.jatuh_tempo)
			);
			if (result) {
				console.log(
					`Created loan with ID: ${result.loanId} and transaction ID: ${result.transactionId} and notification ID: ${result.notificationId}`
				);
			}
		}
		...
		async function createGlobalNotification(notification) {
			try {
				const { data, error } = await supabase.rpc(
					"admin_create_global_notification",
					{
				...
				console.log(
					`Created global notification with ID: ${data}`
				);
				return data;
			} catch (error) {
				console.error(
					"Error in createGlobalNotification:",
					error
				);
				return null;
			}
		...
		for (const notification of globalNotifications) {
			...
			if (notificationId) {
				console.log(
					`Successfully created global notification: ${notification.judul}`
				);
			}
		}
