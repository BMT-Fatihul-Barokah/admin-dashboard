...
export function KetuaDashboard() {
	...
	const handleDownloadPDF = async (report: Report) => {
      ...
      `;
			...
			const blob = new Blob([dummyPdfContent], {
	...
	const fetchSystemAlerts = async () => {
		...
		try {
			...
			const { data: overdueLoans, error: overdueError } =
				await supabase
					.from("pembiayaan")
					.select(
						"id, anggota_id, jumlah, jatuh_tempo, created_at"
					)
					.eq("status", "aktif")
					.lt(
						"jatuh_tempo",
			...
			if (dashboardData.financialSummary) {
				...
				const currentIncome =
					dashboardData.financialSummary.totalIncome;
				const achievementPercentage =
					(currentIncome / targetIncome) * 100;

				if (achievementPercentage >= 100) {
					alerts.push({
						id: "alert-income-target",
						title: "Target Pendapatan Tercapai",
						description: `Target pendapatan bulan ini telah tercapai ${achievementPercentage.toFixed(
							0
						)}%`,
						severity: "success",
						created_at: new Date().toISOString(),
					...
					alerts.push({
						...
						description: `Target pendapatan bulan ini telah mencapai ${achievementPercentage.toFixed(
							0
						)}%`,
						severity: "info",
						created_at: new Date().toISOString(),
			...
			const { data: problematicLoans, error: problematicError } =
				await supabase
					.from("pembiayaan")
					.select("id")
			...
			const { data: newMembers, error: newMembersError } =
				await supabase
					.from("anggota")
					.select("id")
					.gt(
	...
	useEffect(() => {
		async function fetchDashboardData() {
			...
			try {
				...
				const [financialSummary, memberStats, loanStats] =
					await Promise.all([
				...
				const { data: prevMonthLoans, error: prevLoansError } =
					await supabase
						.from("pembiayaan")
						.select("jumlah")
						.gte(
				...
				const newLoansAmount = currentMonthLoans.reduce(
					(sum, loan) => sum + Number(loan.jumlah),
					0
				);
				const newLoansCount = currentMonthLoans.length;
				const prevLoansAmount = prevMonthLoans.reduce(
					(sum, loan) => sum + Number(loan.jumlah),
					0
				);
				const newLoansChange = calculatePercentageChange(
					prevLoansAmount,
					newLoansAmount
				);
				...
				const {
					data: prevMonthMembers,
					error: prevMembersError,
				} = await supabase
					.from("anggota")
					.select("id")
					.gte(
				...
				const newMembersChange = calculatePercentageChange(
					prevMonthMembersCount,
					newMembersCount
				);
				...
				const reports: Report[] = [
					{
						...
						description:
							"Ringkasan keuangan dan operasional bulan " +
							format(currentDate, "MMMM yyyy", {
								locale: id,
							}),
					...
					},
					{
						...
						title:
							...
							currentDate.getFullYear(),
						description:
							"Ringkasan keuangan dan operasional triwulan " +
							Math.ceil(
								(currentDate.getMonth() + 1) / 3
							) +
							" " +
					...
					},
					{
						...
						title:
							...
							(currentDate.getFullYear() - 1),
						description:
							"Ringkasan keuangan dan operasional tahun " +
							(currentDate.getFullYear() - 1),
						period:
							"Tahun " +
							(currentDate.getFullYear() - 1),
				...
				console.log(
					"Ketua dashboard data fetched with performance metrics"
				);
				...
				console.error(
					"Error fetching ketua dashboard data:",
					error
				);
			} finally {
				setIsLoading(false);
							...
							<div>
								...
								<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
									...
									<div className="bg-amber-50 dark:bg-amber-900/20 p-4 rounded-lg">
										...
										<p className="text-xl font-bold">
											{dashboardData.financialSummary
												? dashboardData.financialSummary.profitMarginRatio.toFixed(
														1
												  ) +
												  "%"
												: "-"}
								...
								<div className="space-y-4">
									<div className="grid grid-cols-2 gap-4">
										...
										<div className="space-y-2">
											...
											<p className="text-2xl font-bold">
												{formatCurrency(
													dashboardData.newLoansAmount
												)}
											</p>
											<div
												className={`flex items-center text-sm ${
													dashboardData.newLoansChange >=
													0
														? "text-amber-600"
														: "text-red-600"
												}`}
												...
												<div className="flex items-center">
													...
													<span className="font-medium">
														...
														0
															? "+"
															: ""}
														{dashboardData.newLoansChange.toFixed(
															1
														)}
										...
										<div className="space-y-2">
											...
											<p className="text-2xl font-bold">
												{
													dashboardData.newMembersCount
												}
											</p>
											<div
												className={`flex items-center text-sm ${
													dashboardData.newMembersChange >=
													0
														? "text-green-600"
														: "text-red-600"
												}`}
												...
												<div className="flex items-center">
													...
													<span className="font-medium">
														...
														0
															? "+"
															: ""}
														{dashboardData.newMembersChange.toFixed(
															1
														)}
									...
									{dashboardData.recentActivities.map(
										(
											...
											return (
												...
												>
													<div
														className={`mt-0.5 rounded-full p-1 ${bgColor}`}
													>
														<Icon
						...
						<CardContent className="h-[400px] overflow-y-auto">
							{isLoadingAlerts ? (
								<div className="flex justify-center items-center h-40">
									...
									.length > 0 ? (
								<div className="space-y-4">
									{dashboardData.systemAlerts.map(
										(alert) => {
											...
											return (
												<div
													...
													className={`flex items-start gap-4 rounded-lg border ${borderColor} ${bgColor} p-4`}
												>
													<div
														className={`mt-0.5 rounded-full p-1 ${iconBgColor}`}
													>
														<Icon
															className={`h-4 w-4 ${iconColor}`}
														/>
													...
													<div className="flex-1">
														...
														<p
															className={`text-sm ${descriptionColor} mt-1`}
														>
															{
																alert.description
															}
														...
														<p
															className={`mt-1 text-xs ${timeColor}`}
														>
															{formatRelativeTime(
																alert.created_at
															)}
													...
													{alert.link && (
														...
														>
															<Button
																...
																} ${buttonBgColor} ${buttonHoverColor} ${buttonTextColor}`}
															>
																{alert.linkText ||
																	"Lihat Detail"}
															</Button>
