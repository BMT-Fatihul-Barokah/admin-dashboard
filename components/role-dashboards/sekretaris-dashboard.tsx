	...
	useEffect(() => {
		async function fetchDashboardData() {
			...
			try {
				...
				console.error(
					"Error fetching sekretaris dashboard data:",
					error
				);
			} finally {
				setIsLoading(false);
									...
									<h3 className="text-lg font-medium text-slate-800 dark:text-slate-200">
										{isLoading ? (
											<span>
												Anggota
												Aktif{" "}
												<Skeleton className="inline-block h-6 w-12 align-middle ml-2" />
											</span>
										) : (
											<span>
												Anggota
															...
															<div className="mt-4 pt-3 border-t border-slate-200 dark:border-slate-700 flex justify-end">
																...
																>
																	<Button
																		variant="outline"
																		size="sm"
																		className="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 hover:bg-blue-50 dark:hover:bg-blue-900 border-blue-200 dark:border-blue-800"
																	>
